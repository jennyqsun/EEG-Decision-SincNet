...
class SincDriftBoundAttChoice_fc(nn.Module):
    ...
    def __init__(self, dropout):
        super(SincDriftBoundAttChoice_fc, self).__init__()
        ...
        self.b_drift_bound =  nn.BatchNorm2d(self.num_filters, momentum=0.99)
        self.b_choice = nn.BatchNorm2d(self.num_filters, momentum=0.99)

        self.separable_conv_drift_bound = SeparableConv2d(self.num_filters, self.num_filters, depth = self.spatialConvDepth, kernel_size= (self.num_chan,1))
        ...
        self.b2_drift_bound = nn.BatchNorm2d(self.num_filters*self.spatialConvDepth, momentum=0.99)
        ...
        self.pool1_drift_bound = nn.AvgPool2d((1, self.pool_window), stride=(1, self.stride_window))  # spatial activation of 100ms and with a stride of 13ms
        ...
        self.dropout1_drift_bound = torch.nn.Dropout(p=dropout)
    ...
    def forward(self, x):
        ...
        score0_drift_bound_ = self.b_drift_bound(score_new_drift_bound)
        score0_drift_bound = self.separable_conv_drift_bound(score0_drift_bound_) # output is [n, 64,1,1870)
        ...
        # relu  layers
        score_drift_bound = self.b2_drift_bound(score0_drift_bound)
        score_drift_bound = F.relu(score_drift_bound)

        score_drift_bound = self.pool1_drift_bound(score_drift_bound)
        score_drift_bound = self.dropout1_drift_bound(score_drift_bound)  # output is [n, 64,1,,37)
        ...
        score2_drift_bound = score_drift_bound.view(-1,self.num_filters*1*self.output_size)  # output [batch size, 64*17)
        score3_drift= self.fc_drift(score2_drift_bound)# output is [batch size, 1]



        # ouput layer for bound
        score2_bound =score_drift_bound.view(-1,self.num_filters*self.spatialConvDepth*self.output_size)
